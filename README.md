...
[image1]: /output_images/calibration_points_13.jpg "Sample calibration chessboard image with detected corners"
...
Two functions are used for camera calibration: `get_points` and `get_calibration`. Both of these functions are in the `defs.py` file. 
...
The next step is to prepare the functions used in color transforms, gradients, etc., that will help us create a thresholded binary image. These functions are in `defs.py` file and here is a table:
...
The code for my perspective transform is included in main `pipeline()` function in `defs.py` file between lines 281 and 297. This section uses the thresholded image (`combined`), as well as source (`src`) and destination (`dst`) points.  I chose to hardcode the source and destination points in the following manner:

```python
box_width = 0.645 # fraction of the bottom edge length of the trapezoid w.r.t image width
mid_width = 0.118 # fraction of the top edge length of the trapezoid w.r.t image width
height_pct = 0.645 # fraction of the top edge veritcal position of the trapezoid w.r.t image height measured from top
bottom_trim = 0.934 # fraction of the bottom edge veritcal position of the trapezoid w.r.t image height measured from top
...
| Source                          | Destination   | 
|:-------------------------------:|:-------------:| 
| 564.47998047, 464.3999939       | 224, 0        | 
| 715.52001953, 464.3999939       | 1056, 0       |
| 1052.80004883, 672.47998047     | 1056, 720     |
| 227.19999695, 672.47998047      | 224, 720      |

![Source points for perspective transform][image5]
...
The class is initialized in the main `pipeline` function in `defs.py` at line 321. For the test video, these values are used:
...
The numerical calculation for this part is again given in the main `pipeline()` function in `defs.py` (429). 
...
To process video files we use `moviepy` package. The code is provided at the bottom of the `defs.py` function.
...
<video controls="controls">
  <source type="video/mp4" src="output_videos/project_video_processed.mp4"></source>
  <p>Your browser does not support the video element. "output_videos/project_video_processed.mp4"</p>
</video>
